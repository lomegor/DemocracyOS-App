...
app.get('/tag/all', function (req, res) {
  ...
  api.tag.all(function (err, tags) {
    ...
    res.json(tags.map(expose('id hash name expert.id expert.email color image createdAt')));
  });
});
...
app.get('/tag/:id', function (req, res) {
  ...
  api.tag.get(req.params.id, function (err, tagDoc) {
    ...
    var keys = [
      'id hash name expert.id expert.email color image createdAt'
    ].join(' ');
...
app.post('/tag/create', staff, function (req, res, next) {
  ...
  api.tag.create(req.body, function (err, tagDoc) {
    ...
    var keys = [
      'id hash expert.email name color image createdAt'
    ].join(' ');
