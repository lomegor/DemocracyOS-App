...
export default connect.defaults({
  ...
  commentsSync.setExperts(props.topic.tag.experts)
...
function commentsSyncFactory () {
  ...
  const sync = {
    ...
    setExperts (val) {
      if (params.expertsIds === val) return

      sync.clear()
      params.expertsIds = val
    },
    ...
    sortByExperts (comments) {
      comments.sort((a, b) => params.expertsIds.includes(a.author.id) ? false : params.expertsIds.includes(b.author.id) ? true : false)
      return comments
    },
    ...
    all (body) {
      ...
      items = items.concat(sync.sortByExperts(body.results.comments))
