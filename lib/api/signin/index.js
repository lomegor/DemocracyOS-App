  ...
  auth(email, req.body.password, function (err, user, info) {
    ...
    if (!user) {
      return User.findByEmail(email, function (err, user) {
        if (err) {
          console.error(err)
          return res.json(500, { error: t('modals.error.default', req.locale) })
        }
        ...
        if (
          ...
          return res.json(200, { error: t(info.message, req.locale) })
        }
      })
    }
    ...
    if (user.disabledAt) {
      return res.json(200, { error: t('signin.account-disabled', req.locale) })
    }
