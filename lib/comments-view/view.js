...
export default class CommentsView extends FormView {
  ...
  constructor (topic) {
    ...
    this.topic = topic;
    this.isExpert = topic.tag.expert === user.id;
    this.page = 0;
  ...
  initializeComments () {
    ...
    .query({ exclude_user: user.id && [user.id, this.topic.tag.expert].join('.') || this.topic.tag.expert })
    .end((err, res) => {
      if (err) {
  ...
  /**
   * Load topic expert's comments
   *
   * @api public
   */

  initializeExpertComments () {
    var view = this;
    request
      .get(this.url() + '/expert-comments')
      .query({ expert: this.topic.tag.expert })
      .end((err, res) => {
        if (err) {
          log('Fetch error: %s', err);
          return;
        ...
        view.emit('fetch expert comments', res.body);
      });
  }
  ...
  switchOn () {
    ...
    this.on('fetch expert comments', this.bound('loadExpertComments'));
    this.on('post', this.isExpert ? this.bound('addexpertcomment') : this.bound('addmycomment'));
  ...
  /**
   * Load user's comments in view's `el`
   *
  ...
  /**
   * Load expert's comments in view's `el`
   *
   * @param {Array} comments
   * @api public
   */

  loadExpertComments (comments) {
    if (comments.length) {
      comments.forEach(this.bound('addexpertcomment'));
    }
  };
  ...
  addexpertcomment (comment) {
    let card = new CommentCard(comment);
    let container = this.find('.expert-comments-list')[0];
    this.expertcomments.push(comment);
    card.appendTo(container);
    card.on('delete', this.bound('removeexpertcomment'));
  }

  removeexpertcomment (comment) {
    let i = this.expertcomments.indexOf(comment);
    this.expertcomments.splice(i, 1);
    if (!this.expertcomments.length) {
  ...
  onsuccess (res) {
    this.isExpert ? this.addexpertcomment(res.body) : this.addmycomment(res.body);
    this.hideNewComment();
    this.track(res.body);
  }
