...
exports.update = function update (id, data, fn) {
  ...
  function onget (err, tag) {
    ...
    ensureExpert(data, function(err, newData) {
      if (err) {
...
exports.searchOne = function searchOne (query, fn) {
  ...
  Tag.findOne({ hash: hashQuery }, function (err, tag) {
    ...
    User.populate(tag, { path: 'expert' }, function(err) {
      if (err) {
        log('Found error %s', err)
        return fn(err);
      };
...
exports.create = function create (tag, fn) {
  ...
  if (!tag.expert) {
    ...
    var errMsg = 'Expert should be provided for tag ' + tag.name;
    ...
    if ("string" !== typeof tag.expert) {
      tag.expert = tag.expert.email;
    }
  }
  ...
  ensureExpert(tag, function(err, data) {
    if (err) {
      log('Found error %s', err)
      return fn(err);
    }
    ...
    tag.save(function (err, saved) {
      if (err) {
        if (11000 == err.code) {
          ...
          exports.searchOne(data.hash, fn);
        } else {
          log('Found error %j', err);
          fn(err);
        }
      ...
      User.populate(saved, { path: 'expert' }, function(err) {
        if (err) {
          log('Found error %s', err)
          return fn(err);
        };
...
function ensureExpert(data, fn) {
  userApi.getByEmail(data.expert, function(err, user) {
    if (err) {
      log('Found error %s', err.message)
      return fn(err);
    }
    ...
    if (user) {
      log('Assigning user %s to tag %j', user.id, data.name)

      data.expert = user;
      fn(null, data);
    } else {
      var err = {};
      err.message = 'No expert with email ' + data.expert + ' found.';
      err.name = 'Validation error.';
