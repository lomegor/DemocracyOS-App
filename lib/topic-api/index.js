...
app.get('/topic/:id/comments', function (req, res) {
  ...
  var paging = {
    ...
    sort: sort,
    exclude_user: req.query.exclude_user && req.query.exclude_user.split(".") || null
  };
...
app.get('/topic/:id/expert-comments', function (req, res) {
  log('Request /topic/%s/expert-comments', req.params.id);

  api.topic.userComments(req.params.id, req.query.expert, function (err, comments) {
    if (err) return _handleError(err, req, res);

    log('Serving topic %s comments %j for expert %s', req.params.id, pluck(comments, 'id'), req.query.expert);
